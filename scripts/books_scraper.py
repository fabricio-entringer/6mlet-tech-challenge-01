...
class BooksScraper:
    ...
    def _extract_book_data(self, book_element: BeautifulSoup, category: str) -> Dict[str, str]:
        """
    ...
    def _scrape_category_pages(self, category_url: str, category_name: str) -> List[Dict[str, str]]:
        """
        ...
        while current_url:
            try:
                ...
                self.logger.error(f"Error scraping category {category_name} page {page_num}: {e}")
                break
    ...
    def scrape_all_books(self, execution_type: str = "DIRECT") -> List[Dict[str, str]]:
        ...
        try:
            ...
            if not categories:
                ...
                self._log_execution_history(execution_type, 0, 0, "", status, error_message)
                return []
            ...
            if len(all_books) > 0:
                status = "SUCCESS"
                self.logger.info(f"Scraping completed successfully. Total books collected: {len(all_books)}")
            else:
                status = "FAILED"
            ...
            total_categories = len(set(book.get('category', '') for book in self.books_data)) if self.books_data else 0
    ...
    def save_to_csv(self, filename: str = "books_data.csv", output_dir: str = "data") -> str:
        """
    ...
    try:
        ...
        if books:
            ...
            sorted_categories = sorted(
                stats['categories_breakdown'].items(),
                key=lambda x: x[1],
                reverse=True
            )
