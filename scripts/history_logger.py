...
class ScrapingHistoryLogger:
    ...
    def __init__(self, output_dir: str = None) -> None:
        ...
        self.history_file = os.path.join(
            self.output_dir, SCRAPER_CONFIG["history_log_filename"]
        )
    ...
    def _initialize_history_file(self) -> None:
        ...
        if not os.path.exists(self.history_file):
            try:
                with open(
                    self.history_file, "w", newline="", encoding="utf-8"
                ) as csvfile:
    ...
    def get_history_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_books = sum(
                int(row["total_books_scraped"])
                for row in rows
                if row["total_books_scraped"].isdigit()
            )
            ...
            return {
                ...
                "partial_executions": total_executions
...
def create_configuration_snapshot(
    delay: float, max_retries: int, timeout: int, base_url: str = None
) -> Dict[str, Any]:
    """
