...
async def run_scraping_task(
    ...
    try:
        ...
        result = api.scrape_all_books(
            save_csv=True,
            csv_filename=None  # Use default filename from config
        )
...
async def get_scraping_history(
    ...
    try:
        ...
        if "error" in summary:
            raise HTTPException(
                status_code=500,
                detail=f"Error reading history: {summary['error']}"
            )
...
async def get_scraping_status() -> StatusResponse:
    ...
    return StatusResponse(
        ...
        message="Scraping in progress" if current_scraping_status["is_running"] else "No active scraping operation"
    )
